...
def take_cmd_params(argv):
    parser = argparse.ArgumentParser(description="This script continues model training",
                                     epilog="E.g. " + sys.argv[0] + " ",
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    parser.add_argument("--data-dir", default='', type=str, help="Train data directory", required=True)
    parser.add_argument("--model-in", default='', type=str, help="Model number to start", required=True)
    parser.add_argument("--model-dir", default='', type=str, help="Model save directory", required=True)
...
def main(argv):
    ...
    with params.strategy.scope():
        print(f'Loading model {args.model_in}')
        m = model.load(args.model_in)
        m.summary(150)
        trainT.train_model(m, params)
